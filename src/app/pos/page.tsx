...
export default function POSPage() {
  ...
  // Reusable Order Panel Content to maintain parity between Desktop column and Mobile Drawer
  const orderPanelContent = (
    <div className="flex flex-col flex-1 min-h-0 bg-white dark:bg-[#0f1117]">
      {/* Cart Header */}
      ...
      {/* Customer Search */}
      <div className="p-4 border-b border-slate-200/60 dark:border-white/5 space-y-2 shrink-0">
        <div className="relative">
          ...
          <input type="text" placeholder="Search Customer (Phone/Name)..."
            ...
            className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl pl-10 pr-10 py-2.5 text-xs outline-none focus:border-orange-500 transition-all font-medium text-slate-900 dark:text-white" />
          {customer && (
      ...
      {/* Summary & Checkout */}
      <div className="bg-slate-50/50 dark:bg-[#12141c] p-4 border-t border-slate-200/60 dark:border-white/5 space-y-3 overflow-y-auto shrink-0">
        {/* Paid Amount + Change */}
        <div className="bg-white dark:bg-[#1c1f2a] rounded-xl p-3 border border-slate-200/60 dark:border-white/5">
            ...
            <div className="flex-1">
              ...
              <div className="relative">
                ...
                <input type="number" placeholder="500" value={paidAmount} onChange={(e) => setPaidAmount(e.target.value)}
                  className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl pl-7 pr-3 py-2 text-base font-black outline-none focus:border-orange-500 transition-all text-slate-900 dark:text-white" />
              </div>
            </div>
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex justify-between text-xs font-bold text-slate-500 uppercase tracking-widest">
            <span className="flex items-center gap-1.5"><Percent size={11} className="text-orange-500" /> Tax (GST {cart.length > 0 ? "per item" : "0%"})</span>
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="relative w-28">
              ...
              <input type="number" min="0" placeholder="0" value={discount} onChange={(e) => setDiscount(e.target.value)}
                className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-lg pl-6 pr-2 py-1.5 text-xs font-black outline-none focus:border-emerald-400 transition-all text-right text-slate-900 dark:text-white" />
            </div>
          </div>
          ...
          <div className="flex justify-between items-end pt-3 border-t border-slate-200 dark:border-white/10">
            <div>
              ...
              <h2 className="text-2xl font-black text-slate-900 dark:text-white tracking-tighter">₹{total.toLocaleString()}</h2>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <select
            ...
            className={clsx(
              ...
              accounts.length === 0 ? "border-rose-500 text-rose-500" : "border-slate-200 dark:border-white/10 focus:border-orange-500 text-slate-800 dark:text-white"
            )}
          >
            {accounts.length === 0 ? (
          ...
          {( [
            ...
            return (
              <button 
                ...
                disabled={isDisabled}
                className={clsx(
                  "flex flex-col items-center gap-1 py-2.5 rounded-xl border transition-all relative",
        ...
        {accounts.length === 0 ? (
          <button 
            onClick={() => setShowAddAccount(true)}
            className="w-full h-12 bg-rose-500 hover:bg-rose-600 text-white rounded-xl font-black text-xs uppercase tracking-[0.2em] shadow-lg shadow-rose-500/20 active:scale-[0.98] transition-all flex items-center justify-center gap-3 animate-in fade-in zoom-in duration-300"
          >
            <Banknote size={18} />
          ...
          <button onClick={() => handleCheckout(selectedPaymentMode)}
            ...
            className="w-full h-12 bg-orange-500 hover:bg-orange-600 disabled:bg-slate-300 dark:disabled:bg-white/5 disabled:cursor-not-allowed text-white rounded-xl font-black text-sm uppercase tracking-[0.2em] shadow-lg shadow-orange-500/20 active:scale-[0.98] transition-all flex items-center justify-center gap-2"
          >
            {loading ? (
  ...
  return (
    <div className="-m-6 flex h-[calc(100vh-3.5rem)] overflow-hidden bg-[#F8FAFC] dark:bg-[#090a0f] relative">

      {/* LEFT: PRODUCT AREA */}
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="bg-white dark:bg-[#12141c] px-4 sm:px-6 py-4 flex flex-col gap-3 border-b border-slate-200/60 dark:border-white/5 shrink-0">
          <div className="flex flex-col sm:flex-row sm:items-center gap-4">
            <div className="flex bg-slate-100 dark:bg-white/5 p-1 rounded-2xl gap-1 shrink-0 w-full sm:w-auto">
              {(["counter", "wholesale", "delivery"] as const).map((t) => (
                <button key={t} onClick={() => setOrderType(t)}
                  className={clsx("flex-1 sm:flex-none px-4 sm:px-5 py-2 sm:py-2.5 rounded-xl text-[10px] sm:text-xs font-black uppercase tracking-wider transition-all text-center",
            ...
            <div className="relative flex-1 w-full group">
              ...
              <input ref={searchRef} type="text" placeholder="Search products (Space to focus)..."
                ...
                className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl pl-12 pr-6 py-2.5 sm:py-3 text-sm font-medium outline-none focus:ring-4 focus:ring-orange-500/10 focus:border-orange-500 transition-all text-slate-900 dark:text-white" />
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-3 sm:p-6 bg-slate-50/50 dark:bg-[#090a0f] pb-28 lg:pb-6">
          {productsLoading ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-3 sm:gap-4">
                ...
                return (
                  <div key={item.id} 
                    ...
                    <div className="flex items-center justify-between gap-2 mb-2">
                      <div className="w-7 h-7 rounded-lg bg-slate-50 dark:bg-white/5 flex items-center justify-center text-sm shadow-sm border border-slate-100 dark:border-white/5">
                        {item.emoji}
                      </div>
                      <div className="flex items-center gap-1.5 px-2 py-1 rounded-full bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 shrink-0">
                        <span className={clsx("w-1.5 h-1.5 rounded-full", isOutOfStock ? "bg-rose-500" : "bg-emerald-500 shadow-[0_0_6px_rgba(16,185,129,0.5)]")} />
                        <span className="text-[8px] font-black uppercase tracking-tight text-slate-500 dark:text-slate-400">
                    ...
                    {qtyInCart > 0 && (
                      <div onClick={(e) => e.stopPropagation()}
                        className="absolute inset-x-2 sm:inset-x-3 bottom-2 sm:bottom-3 flex items-center justify-between bg-orange-500 rounded-xl p-1 shadow-md animate-in fade-in slide-in-from-bottom-1 duration-200">
                        <button onClick={() => updateQty(item.id, -1)} className="w-7 h-7 sm:w-9 sm:h-9 flex items-center justify-center text-white hover:bg-white/20 rounded-lg transition-all"><Minus size={14} strokeWidth={3} /></button>
                        <span className="text-xs sm:text-sm font-black text-white">{qtyInCart}</span>
                        <button onClick={() => updateQty(item.id, 1)} className="w-7 h-7 sm:w-9 sm:h-9 flex items-center justify-center text-white hover:bg-white/20 rounded-lg transition-all"><Plus size={14} strokeWidth={3} /></button>
                      </div>
                    )}
                  </div>
                );
        ...
        <footer className="bg-white dark:bg-[#12141c] border-t border-slate-200/60 dark:border-white/5 py-3 px-4 sm:px-6 flex items-center justify-between shrink-0">
      ...
      <div className="hidden lg:flex w-[340px] xl:w-[380px] 2xl:w-[420px] bg-white dark:bg-[#0f1117] border-l border-slate-200/60 dark:border-white/5 flex-col shrink-0">
        {orderPanelContent}
      </div>

      {/* MOBILE/TABLET FLOATING ACTION BUTTON */}
      {cart.length > 0 && (
        <div className="fixed bottom-6 inset-x-6 z-40 lg:hidden animate-in slide-in-from-bottom-5 duration-300">
          <button
            onClick={() => setShowCartDrawer(true)}
            className="w-full h-14 bg-gradient-to-r from-orange-500 to-amber-500 text-white rounded-2xl font-black text-xs sm:text-sm uppercase tracking-widest shadow-xl shadow-orange-500/30 flex items-center justify-between px-6 active:scale-95 transition-all"
          >
            <div className="flex items-center gap-3">
      ...
      {/* MOBILE DRAWER */}
      <div className={clsx(
        "fixed inset-0 z-[110] bg-black/60 backdrop-blur-sm lg:hidden transition-opacity duration-300",
        showCartDrawer ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"
      )} onClick={() => setShowCartDrawer(false)} />
      <div className={clsx(
        "fixed inset-y-0 right-0 z-[120] w-full max-w-md bg-white dark:bg-[#0f1117] shadow-2xl flex flex-col transition-transform duration-300 transform lg:hidden",
        showCartDrawer ? "translate-x-0" : "translate-x-full"
      )}>
        <div className="p-4 border-b border-slate-100 dark:border-white/5 flex items-center justify-between shrink-0 bg-slate-50 dark:bg-slate-900/50">
          <span className="text-xs font-black uppercase tracking-widest text-slate-400">Review & Pay</span>
          <button onClick={() => setShowCartDrawer(false)} className="p-2 hover:bg-slate-100 dark:hover:bg-white/5 rounded-xl text-slate-400">
            <X size={18} />
          </button>
        </div>
        {orderPanelContent}
          ...
          <div className="bg-white dark:bg-[#12141c] rounded-[2.5rem] shadow-2xl w-full max-w-sm p-8 space-y-6">
              ...
              {[
                ...
                <div key={field}>
                  ...
                  <input 
                    ...
                    className="w-full px-5 py-3.5 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl font-semibold text-sm outline-none focus:border-orange-400 transition-all dark:text-white" />
                </div>
              ))}
            </div>
            <div className="flex gap-3">
              ...
              <button onClick={handleAddCustomer}
                ...
                className="flex-[2] py-3.5 bg-orange-500 hover:bg-orange-400 disabled:bg-slate-300 text-white rounded-2xl text-sm font-black flex items-center justify-center gap-2 shadow-lg shadow-orange-500/20 transition-all active:scale-95 text-slate-900 dark:text-white">
