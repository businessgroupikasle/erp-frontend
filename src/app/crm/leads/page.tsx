            ...
            <table className="w-full text-left border-collapse min-w-[1600px]">
              <thead>
                <tr className="bg-white dark:bg-slate-900 border-b border-[#F0EAF0] dark:border-slate-800">
                  ...
                  {["Pipeline", "Contact Name", "Organisation", "Email", "Phone", "Country", "City", "Created At", "Lead Source", "Budget", "Subject", "Assignee", "Follow Up Date", "Status"].map((head) => (
                ...
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={15} className="py-40">
                      <div className="flex flex-col items-center justify-center space-y-6 animate-in fade-in zoom-in duration-500">
                        ...
                        <Link href="/crm/leads/new" className="px-6 py-2 bg-[#7C3AED] text-white rounded-lg font-bold text-[13px]">Add Your First Lead</Link>
                      </div>
                    </td>
                  </tr>
                ) : (
                  leads.map((lead) => (
                    <tr key={lead.id} className="border-b border-[#F0EAF0] dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                      ...
                      <td className="p-4 text-[13px] font-medium text-[#666] dark:text-slate-400 whitespace-nowrap">{lead.pipeline?.name || "—"}</td>
                      <td className="p-4 text-[13px] font-bold text-[#1A1A1A] dark:text-white whitespace-nowrap">{lead.contactName}</td>
                      <td className="p-4 text-[13px] text-[#666] dark:text-slate-400 whitespace-nowrap">{lead.orgName || "—"}</td>
                      <td className="p-4 text-[13px] text-[#666] dark:text-slate-400 whitespace-nowrap">{lead.email || "—"}</td>
                      <td className="p-4 text-[13px] text-[#666] dark:text-slate-400 whitespace-nowrap">{lead.phone || "—"}</td>
                      <td className="p-4 text-[13px] text-[#666] dark:text-slate-400 whitespace-nowrap">{lead.contactCountry || "—"}</td>
                      <td className="p-4 text-[13px] text-[#666] dark:text-slate-400 whitespace-nowrap">{lead.customerCity || "—"}</td>
                      <td className="p-4 text-[13px] text-[#666] dark:text-slate-400 whitespace-nowrap">{new Date(lead.createdAt).toLocaleDateString()}</td>
                      <td className="p-4 text-[13px] text-[#666] dark:text-slate-400 whitespace-nowrap">{lead.leadSource || "—"}</td>
                      <td className="p-4 text-[13px] text-[#666] dark:text-slate-400 whitespace-nowrap">{lead.budget ? `₹${lead.budget.toLocaleString()}` : "—"}</td>
                      <td className="p-4 text-[13px] text-[#666] dark:text-slate-400 whitespace-nowrap">{lead.subject || "—"}</td>
                      <td className="p-4 text-[13px] text-[#666] dark:text-slate-400 whitespace-nowrap">{lead.assigneeId || "—"}</td>
                      <td className="p-4 text-[13px] text-[#666] dark:text-slate-400 whitespace-nowrap">{lead.followUpDate ? new Date(lead.followUpDate).toLocaleDateString() : "—"}</td>
                      <td className="p-4">
