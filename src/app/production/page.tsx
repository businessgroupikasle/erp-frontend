...
interface IngredientPreview { id: string; name: string; unit: string; needed: string; available: number | string; sufficient: boolean; costPrice: number; }
...
function ProductionContent() {
  ...
  const [selectedCatalogRecipe, setSelectedCatalogRecipe] = useState<any>(null);
  ...
  const [productionType] = useState("FINISHED_GOOD");
  ...
  const fetchAll = useCallback(async () => {
    setLoading(true);
    try {
      const [hRes, rRes, fRes, mRes] = await Promise.all([
  ...
  const ingredientPreview: IngredientPreview[] = selectedRecipe?.recipeItems?.map((item: any) => {
    ...
    const materialCost = allMaterials.find(m =>
      m.id === item.inventoryItemId ||
      m.sku?.toLowerCase() === item.inventoryItem?.sku?.toLowerCase()
    )?.costPrice || 0;
  ...
  // Production cost = sum of (qty_needed * cost_per_unit) for each ingredient
  const totalProductionCost = ingredientPreview.reduce((sum, ing) => {
    return sum + ((parseFloat(ing.needed) || 0) * (ing.costPrice || 0));
  }, 0);
  const totalUnitsProduced = quantity * targetUnits;
  const costPerUnit = totalUnitsProduced > 0 ? totalProductionCost / totalUnitsProduced : 0;
    ...
    <div className="max-w-7xl mx-auto space-y-4 md:space-y-8 animate-in fade-in duration-700 px-4 sm:px-0">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 py-3 md:py-4 border-b border-slate-200 dark:border-slate-800">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => setShowRecipeCatalog(true)}
            className="flex items-center gap-2 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-200 px-4 py-3 md:py-3.5 rounded-xl md:rounded-2xl font-black text-[9px] md:text-[10px] uppercase tracking-widest hover:border-orange-500 hover:text-orange-500 transition-all active:scale-[0.98] shadow-sm shrink-0"
          >
            <ChefHatIcon size={14} className="text-[#F97316]" />
            <span className="hidden sm:inline">Formula Catalog</span>
                ...
                <div key={batch.id} className="group bg-white dark:bg-card/40 backdrop-blur-md rounded-[24px] md:rounded-[32px] border border-slate-100 dark:border-white/5 p-4 md:p-5 hover:shadow-2xl transition-all relative overflow-hidden">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="flex items-center gap-5 p-2.5 mt-3 bg-slate-50 dark:bg-white/[0.03] rounded-xl border border-slate-100 dark:border-white/5 max-w-fit">
                          ...
                          <div className="min-w-[100px]">
                            <p className="text-[8px] font-black text-emerald-500 uppercase tracking-[0.1em] mb-0.5">Actual Output</p>
                        ...
                        <div className="flex flex-wrap items-center gap-3 mt-3">
                          ...
                          {batch.expiryDate && (
                            <span className="text-[8px] md:text-[9px] font-black text-amber-500 uppercase tracking-widest flex items-center gap-1.5 bg-amber-50 dark:bg-amber-500/10 px-2.5 py-1 rounded-lg">
                  ...
                  <div className="mt-4 md:mt-5 pt-4 md:pt-5 border-t border-slate-100 dark:border-white/5 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2.5 md:gap-3">
                    {batch.items?.map((item: any, i: number) => (
                      <div key={i} className="p-2.5 md:p-3 bg-slate-50/50 dark:bg-white/[0.01] rounded-lg md:rounded-xl border border-slate-100 dark:border-white/5">
          ...
          <div className="relative bg-white rounded-[2rem] shadow-2xl w-full max-w-lg border border-slate-100 p-6 md:p-8 overflow-hidden max-h-[92vh] overflow-y-auto custom-scrollbar">
            ...
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Number of Batches *</label>
                  <div className="flex items-center gap-2 h-11 bg-slate-50 border border-slate-100 rounded-xl px-3">
                    <button
                      onClick={() => setQuantity(q => Math.max(1, q - 1))}
                      className="w-7 h-7 rounded-lg bg-white border border-slate-200 flex items-center justify-center hover:bg-slate-50 transition-all"
                    >
                      <MinusIcon size={14} />
                    </button>
                    <input
                      ...
                      className="flex-1 bg-transparent text-center font-black text-sm outline-none"
                    />
                    <button
                    ...
                    {[1, 2, 5, 10].map(q => (
                      <button
                        ...
                        className={clsx(
                          ...
                          quantity === q ? "bg-orange-500 text-white" : "bg-slate-100 text-slate-500 hover:bg-slate-200"
                        )}
                      >
                        {q}x
                      </button>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="h-11 bg-orange-50 border border-orange-100 rounded-xl flex items-center px-4 font-black text-orange-600 text-sm">
                    {quantity * targetUnits} Units
                  </div>
                  <p className="text-[9px] text-slate-400 font-bold ml-1">{quantity} batch × {targetUnits} yield = {quantity * targetUnits}</p>
                </div>
              </div>
              ...
              {recipeId && ingredientPreview.length > 0 && (
                <div className="mt-2">
                  <div className="flex items-center justify-between mb-3 px-1">
                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Ingredient Availability</p>
                    ...
                    {ingredientPreview.map((ing) => (
                      <div key={ing.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100">
                        <div className="flex flex-col">
                          ...
                          <span className="text-[9px] font-bold text-slate-400">Need: {ing.needed} {ing.unit}</span>
                        </div>
              ...
              {recipeId && ingredientPreview.length > 0 && totalProductionCost > 0 && (
                <div className="p-4 bg-orange-50 border border-orange-100 rounded-2xl space-y-3">
                  <div className="flex items-center gap-2">
                    <IndianRupeeIcon size={14} className="text-orange-600" />
                    <p className="text-[9px] font-black text-orange-600 uppercase tracking-widest">Production Cost Estimate</p>
                  </div>
                  <div className="space-y-1.5">
                    {ingredientPreview.filter(i => i.costPrice > 0).map(ing => (
                      <div key={ing.id} className="flex justify-between text-[10px] font-bold text-slate-600">
                        <span className="truncate max-w-[180px]">{ing.name} × {ing.needed} {ing.unit}</span>
                        <span className="text-orange-600 font-black">₹{((parseFloat(ing.needed) || 0) * ing.costPrice).toFixed(2)}</span>
                      </div>
                    ))}
                  </div>
                  <div className="pt-2 border-t border-orange-200 flex items-center justify-between">
                    <div>
                      <p className="text-[9px] font-black text-orange-700 uppercase">Total Batch Cost</p>
                      <p className="text-[8px] text-orange-500 font-bold">₹{costPerUnit.toFixed(2)} per unit</p>
                    </div>
                    <span className="text-lg font-black text-orange-600">₹{totalProductionCost.toFixed(2)}</span>
                  </div>
                </div>
              )}
      ...
      {/* Recipe Catalog Slide-over Drawer */}
      {showRecipeCatalog && (
        <div className="fixed inset-0 z-[120] flex justify-end animate-in fade-in duration-200">
          {/* Overlay */}
          <div 
            className="absolute inset-0 bg-slate-900/40 backdrop-blur-sm cursor-pointer" 
            onClick={() => setShowRecipeCatalog(false)} 
          />
          
          {/* Drawer container */}
          <div className="relative w-full max-w-md bg-white dark:bg-slate-900 h-full shadow-2xl flex flex-col border-l border-slate-100 dark:border-white/5 animate-in slide-in-from-right duration-300">
            {/* Header */}
            <div className="p-6 border-b border-slate-100 dark:border-white/5 flex items-center justify-between shrink-0">
              <div className="flex items-center gap-2.5">
                <div className="w-8 h-8 rounded-lg bg-orange-50 dark:bg-orange-500/10 flex items-center justify-center text-[#F97316]">
                  <ChefHatIcon size={16} />
                </div>
                <div>
                  <h3 className="text-sm font-bold text-slate-800 dark:text-slate-200 uppercase tracking-wider">Formula Catalog</h3>
                  <p className="text-[10px] text-slate-400">Browse recipe instructions & bill of materials</p>
                </div>
              </div>
              <button 
                onClick={() => setShowRecipeCatalog(false)}
                className="p-1.5 rounded-lg hover:bg-slate-50 dark:hover:bg-white/5 text-slate-400 hover:text-slate-600 transition-colors"
              >
                <XIcon size={16} />
              </button>
            </div>
            ...
            {/* Search Input */}
            <div className="p-4 border-b border-slate-100 dark:border-white/5 shrink-0 bg-slate-50/50 dark:bg-transparent">
              <div className="relative">
                <input
                  type="text"
                  value={catalogSearch}
                  onChange={e => setCatalogSearch(e.target.value)}
                  placeholder="Search recipe formula..."
                  className="w-full px-3 py-2 border border-slate-200 dark:border-slate-800 rounded-lg outline-none focus:border-orange-500 text-xs font-semibold bg-white dark:bg-slate-950 dark:text-white"
                />
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
              {recipes
                .filter(r => !catalogSearch || r.name?.toLowerCase().includes(catalogSearch.toLowerCase()) || r.product?.name?.toLowerCase().includes(catalogSearch.toLowerCase()))
                .map(r => {
                  const isSelected = selectedCatalogRecipe?.id === r.id;
                  return (
                    <div 
                      key={r.id} 
                      className={clsx(
                        ...
                        isSelected 
                          ? "bg-orange-50/40 dark:bg-orange-500/5 border-orange-200 dark:border-orange-500/20 shadow-sm" 
                          : "bg-white dark:bg-slate-900/20 border-slate-100 dark:border-white/5 hover:border-slate-200 hover:bg-slate-50/30"
                      )}
                    >
                      <div 
                        className="flex items-center justify-between cursor-pointer"
                        onClick={() => setSelectedCatalogRecipe(isSelected ? null : r)}
                      >
                        <div className="flex items-center gap-2.5">
                          <div className={clsx(
                            "w-8 h-8 rounded-lg flex items-center justify-center font-bold text-xs shrink-0 transition-colors",
                            isSelected ? "bg-[#F97316] text-white" : "bg-slate-100 dark:bg-slate-800 text-slate-500"
                          )}>
                            {r.name.substring(0, 2).toUpperCase()}
                          </div>
                          <div>
                            <h4 className="text-xs font-bold text-slate-800 dark:text-slate-200 transition-colors">{r.name}</h4>
                            <p className="text-[9px] text-slate-400 font-bold uppercase mt-0.5">Product: {r.product?.name || "—"}</p>
                          </div>
                        </div>
                        <div className="text-right shrink-0">
                          <p className="text-[10px] font-bold text-slate-700 dark:text-slate-300">{r.yieldQty} Units</p>
                          <p className="text-[8px] text-slate-400 font-semibold uppercase mt-0.5">per batch</p>
                        </div>
                      </div>

                      {/* Expanded Section showing Bill of Materials & Instructions */}
                      {isSelected && (
                        <div className="mt-4 pt-4 border-t border-orange-100 dark:border-white/5 space-y-4 animate-in fade-in duration-300">
                          {/* Ingredients */}
                          <div className="space-y-2">
                            <p className="text-[8px] font-black text-slate-400 uppercase tracking-wider">Bill of Materials</p>
                            <div className="space-y-1.5">
                              {r.recipeItems?.map((item: any, idx: number) => (
                                <div key={idx} className="flex justify-between text-[10px] font-bold text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-white/[0.02] px-2.5 py-1.5 rounded-lg border border-slate-100/50 dark:border-white/5">
                                  <span>{item.inventoryItem?.name || "Ingredient"}</span>
                                  <span className="text-[#F97316] font-black">{item.quantityRequired} {item.unit || "KG"}</span>
                                </div>
                              ))}
                              {(!r.recipeItems || r.recipeItems.length === 0) && (
                                <p className="text-[9px] text-slate-400 italic">No materials defined.</p>
                              )}
                            </div>
                          </div>
                          ...
                          {r.instructions && (
                            <div className="space-y-2">
                              <p className="text-[8px] font-black text-slate-400 uppercase tracking-wider">Instructions</p>
                              <div className="bg-[#FFFDF9] dark:bg-slate-950 p-3 rounded-lg border border-orange-100 dark:border-white/5 text-[10px] font-medium text-slate-700 dark:text-slate-300 leading-relaxed max-h-32 overflow-y-auto whitespace-pre-line custom-scrollbar">
                                {r.instructions.replace(/\[unitWeight:[\d.]+\]/, "").replace(/\[weightUnit:\w+\]/, "").trim()}
                              </div>
                            </div>
                          )}
                          ...
                          <button
                            onClick={() => {
                              setRecipeId(r.id);
                              setTargetUnits(r.yieldQty || 1);
                              setQuantity(1);
                              setShowForm(true);
                              setShowRecipeCatalog(false);
                            }}
                            className="w-full py-2 bg-[#F97316] hover:bg-orange-600 text-white rounded-lg font-bold text-[9px] uppercase tracking-widest shadow-md transition-all active:scale-[0.99] flex items-center justify-center gap-1.5"
                          >
              ...
              {recipes.filter(r => !catalogSearch || r.name?.toLowerCase().includes(catalogSearch.toLowerCase()) || r.product?.name?.toLowerCase().includes(catalogSearch.toLowerCase())).length === 0 && (
                <div className="py-12 text-center">
                  <ChefHatIcon size={32} className="mx-auto text-slate-200 dark:text-slate-800 mb-2" />
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">No recipe formulas found</p>
                </div>
              )}
            </div>
          </div>
