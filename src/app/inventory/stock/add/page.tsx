...
export default function AddMaterialPage() {
  ...
  useEffect(() => {
    if (user?.role === "SUPER_ADMIN") {
      franchiseApi.getAll().then((res) => setFranchises(res.data)).catch(console.error);
    }
    // Always fetch vendors for linking
    rawMaterialsApi.getAll().then(() => {
       import("@/lib/api").then(api => {
          api.vendorsApi.getAll().then(res => setVendors(res.data)).catch(console.error);
       });
    });
  }, [user]);
  ...
  const handleSave = async () => {
    ...
    if (sourceType === "VENDOR" && !form.vendorId) {
      setError("Please select a vendor for this material");
      return;
    }
