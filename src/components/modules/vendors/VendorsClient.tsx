    ...
    <div className="flex h-[calc(100vh-100px)] bg-slate-50 dark:bg-[#0b0c14] -m-4 overflow-hidden selection:bg-orange-500/30 selection:text-orange-500 transition-colors">
      ...
      <div className="flex-1 flex flex-col relative overflow-hidden bg-white dark:bg-[#0b0c14]">
        {selectedVendor ? (
          <>
            <div className="px-6 py-4 border-b border-slate-200 dark:border-white/5 relative z-10 flex items-center justify-between bg-white dark:bg-transparent">
              ...
              <div className="flex items-center gap-2">
                 <button onClick={() => { setEditing(selectedVendor); setForm({ ...selectedVendor }); setShowForm(true); }} className="px-4 py-2 bg-slate-100 dark:bg-white/5 text-slate-700 dark:text-white rounded-xl text-[9px] font-black uppercase border border-slate-200 dark:border-white/10 transition-all">Edit Profile</button>
                 <button onClick={() => { setPaymentForm({ amount: "", note: "", type: "PAYMENT", accountId: accounts[0]?.id || "" }); setShowPaymentModal(true); }} className="px-6 py-2 bg-orange-500 text-white rounded-xl text-[9px] font-black uppercase shadow-lg shadow-orange-500/20 active:scale-95 transition-all">Record Transaction</button>
              </div>
            </div>

            <div className="grid grid-cols-4 gap-4 p-4">
               <div className="p-4 rounded-2xl border bg-white dark:bg-slate-900/40 border-slate-100 dark:border-white/5">
                  <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-1.5"><AlertCircle size={10} className="text-rose-500" /> Total Payable</p>
                  <h3 className="text-2xl font-black text-rose-600 dark:text-rose-400 tracking-tighter">₹{Math.round(selectedVendor.due || 0).toLocaleString()}</h3>
               </div>
               <div className="p-4 bg-white dark:bg-slate-900/40 border border-slate-100 dark:border-white/5 rounded-2xl">
                  <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-1.5"><CheckCircle2 size={10} className="text-emerald-500" /> Total Settled</p>
                  <h3 className="text-2xl font-black text-slate-900 dark:text-white tracking-tighter">₹{Math.round(selectedVendor.totalPaid || 0).toLocaleString()}</h3>
               </div>
               <div className="p-4 bg-white dark:bg-slate-900/40 border border-slate-100 dark:border-white/5 rounded-2xl">
                  <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-1.5"><Wallet size={10} className="text-indigo-500" /> Prepaid Balance</p>
                  <h3 className="text-2xl font-black text-indigo-600 dark:text-indigo-400 tracking-tighter">₹{Math.round(selectedVendor.advance || 0).toLocaleString()}</h3>
               </div>
               <div className="p-4 bg-white dark:bg-slate-900/40 border border-slate-100 dark:border-white/5 rounded-2xl flex flex-col justify-center">
                  ...
                  <div className="flex justify-between items-center text-[7px] font-black uppercase text-slate-400">
                    ...
                    <span className="flex gap-1.5"><span className="text-emerald-500">0-30D</span><span className="text-rose-500">90D+</span></span>
                  </div>
               </div>
            </div>
            ...
            <div className="flex-1 flex flex-col overflow-hidden px-4 min-h-0">
               ...
               <div className={clsx("flex-1 overflow-y-auto custom-scrollbar border border-slate-200 dark:border-white/5 rounded-xl bg-white dark:bg-slate-900/20", ledgerLoading ? "opacity-50" : "opacity-100")}>
                  {ledger.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-full py-10 opacity-40"><FileText size={32} className="mb-2" /><p className="text-[10px] font-black uppercase">No verified transactions</p></div>
                  ) : (
                    <table className="w-full text-left">
                       <thead className="bg-slate-50 dark:bg-slate-800 sticky top-0 z-20">
                          <tr className="text-[8px] font-black text-slate-500 dark:text-slate-400 uppercase border-b border-slate-200 dark:border-white/5">
                          ...
                          {ledger.map(e => {
                            ...
                            return (
                              <tr key={e.id} className="hover:bg-slate-50 dark:hover:bg-white/[0.03] transition-colors text-[10px]">
                                 ...
                                 <td className="py-3">
                                    <div className="flex flex-col">
                                       <span className="font-black text-orange-600 uppercase">{e.referenceType}</span>
                                       <span className="text-slate-500 truncate max-w-[150px]">{e.note || "-"}</span>
                                    </div>
                                 </td>
                                 <td className="py-3"><span className="text-[8px] font-black bg-slate-100 dark:bg-white/5 px-1.5 py-0.5 rounded uppercase">{e.sourceModule}</span></td>
                                 <td className="py-3 text-right"><span className={clsx("font-black", isDebit ? "text-rose-600" : "opacity-0")}>{isDebit ? `₹${Math.round(e.amount).toLocaleString()}` : "—"}</span></td>
                                 <td className="py-3 text-right"><span className={clsx("font-black", !isDebit ? "text-emerald-600" : "opacity-0")}>{!isDebit ? `₹${Math.round(e.amount).toLocaleString()}` : "—"}</span></td>
                                 <td className="py-3 pr-4 text-right">
                                    <div className={clsx("inline-flex flex-col items-end px-2 py-0.5 rounded", balance >= 0 ? "text-emerald-600" : "text-rose-600")}>
                                       <span className="font-black tracking-tighter">₹{Math.abs(Math.round(balance)).toLocaleString()}</span>
                                       <span className="text-[6px] font-black">{balance >= 0 ? "CR" : "DR"}</span>
                                    </div>
                                 </td>
                              </tr>
                            );
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-slate-400 space-y-4"><Store size={48} className="opacity-10" /><p className="text-[10px] font-black uppercase">Select vendor to begin financial analysis</p></div>
        )}
      </div>
